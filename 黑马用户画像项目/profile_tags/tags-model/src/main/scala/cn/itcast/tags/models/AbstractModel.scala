...
abstract class AbstractModel(modelName: String, modelType: ModelType) extends Logging {
  ...
  def getBusinessData(tagDF: DataFrame): DataFrame = {
    ...
    if ("hbase".equals(tagRuleMap("inType").toLowerCase)) {
      ...
      ////        hbaseMeta.selectFieldNames.split(",") // hbase中 politicalface 中的 f 是小写
      //        "id,politicalFace".split(",")
      //      )
...
//        .option("selectFields", "memberId,orderSn,orderAmount,finishTime")
//        .option("selectFields", "memberId,orderSn,orderAmount,couponCodeValue")
//        .option("selectFields", "cOrderSn,ogColor,productType")
