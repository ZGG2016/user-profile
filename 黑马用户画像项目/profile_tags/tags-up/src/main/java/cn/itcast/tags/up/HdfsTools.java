...
public class HdfsTools {
     ...
     * @param dfsPath
     */
    public void list(String dfsPath) {
        FileSystem fs = null;
        try {
            fs = FileSystem.get(new URI(fsAddr), conf, user);
            FileStatus[] fileStatuses = fs.listStatus(new Path(dfsPath));
            for (FileStatus fsStat : fileStatuses) {
     ...
     * @return
     */
    public String readFileInfo(String dfsPath) {
        FileSystem fs = null;
        String jars = "";
        try {
            fs = FileSystem.get(new URI(fsAddr), conf, user);
